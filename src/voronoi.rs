...
impl Pt {
    ...
    #[inline]
    fn mul(self, s: f32) -> Pt { Pt { x: self.x * s, y: self.y * s } }
    #[inline]
    fn dot(self, o: Pt) -> f32 { self.x * o.x + self.y * o.y }
    #[inline]
    fn len2(self) -> f32 { self.dot(self) }
}

#[derive(Clone, Copy)]
struct Circumcircle { c: Pt, r2: f32 }
...
fn circumcircle(a: Pt, b: Pt, c: Pt) -> Option<Circumcircle> {
    ...
    Some(Circumcircle { c: center, r2: center.sub(a).len2() })
}
...
impl Edge {
    fn new(i: usize, j: usize) -> Self {
        if i < j { Edge { a: i, b: j } } else { Edge { a: j, b: i } }
    }
}

#[derive(Clone, Copy)]
struct Tri { a: usize, b: usize, c: usize }
...
fn bowyer_watson(points: &[Pt]) -> Vec<Tri> {
    ...
    for p in points { minx = minx.min(p.x); miny = miny.min(p.y); maxx = maxx.max(p.x); maxy = maxy.max(p.y); }
    let dx = maxx - minx; let dy = maxy - miny; let delta = (dx.max(dy) + 1.0) * 10.0;
    let midx = (minx + maxx) * 0.5; let midy = (miny + maxy) * 0.5;

    let st_a = Pt { x: midx - 2.0 * delta, y: midy - delta };
    let st_b = Pt { x: midx, y: midy + 2.0 * delta };
    let st_c = Pt { x: midx + 2.0 * delta, y: midy - delta };

    let mut pts: Vec<Pt> = points.to_vec();
    let i_a = pts.len();
    let i_b = pts.len() + 1;
    let i_c = pts.len() + 2;
    pts.push(st_a); pts.push(st_b); pts.push(st_c);

    let mut tris = vec![Tri { a: i_a, b: i_b, c: i_c }];

    for (pi, p) in points.iter().enumerate() {
        // 1. Collect triangles whose circumcircle contains p
        let mut bad: Vec<usize> = Vec::new();
        for (ti, t) in tris.iter().enumerate() {
            let a = pts[t.a]; let b = pts[t.b]; let c = pts[t.c];
            if in_circumcircle(*p, a, b, c) { bad.push(ti); }
        }
        ...
        let mut boundary: Vec<Edge> = edge_count.into_iter().filter_map(|(e, c)| if c == 1 { Some(e) } else { None }).collect();
        ...
        for e in boundary.drain(..) {
            tris.push(Tri { a: e.a, b: e.b, c: pi });
        }
    }

    // 5. Remove triangles that include super triangle vertices
    tris.retain(|t| t.a != i_a && t.a != i_b && t.a != i_c &&
                      t.b != i_a && t.b != i_b && t.b != i_c &&
                      t.c != i_a && t.c != i_b && t.c != i_c);
    tris
}
...
fn compute_voronoi_edges(points: &[Pt], tris: &[Tri]) -> Vec<(Pt, Pt)> {
    ...
    impl EdgeKey { fn new(a: usize, b: usize) -> Self { if a < b { Self(a,b) } else { Self(b,a) } } }
    ...
    for t in tris {
        if let Some(cc) = circumcircle(points[t.a], points[t.b], points[t.c]) {
            ...
            let cen = Pt { x: (points[t.a].x + points[t.b].x + points[t.c].x) / 3.0, y: (points[t.a].y + points[t.b].y + points[t.c].y) / 3.0 };
            centers.push(cen);
        }
    }
...
impl VoronoiTests {
    ...
    pub fn voronoi_create_points(count: usize, width: f32, height: f32, seed: u32, speed: f32) -> Vec<f32> {
        ...
        for i in 0..count {
            ...
            let vx = ang.cos() * spd;
            let vy = ang.sin() * spd;
            out.push(x); out.push(y); out.push(vx); out.push(vy);
    ...
    pub fn voronoi_step_points(points: &[f32], width: f32, height: f32, dt: f32) -> Vec<f32> {
        ...
        for i in 0..n {
            ...
            if out_of_bounds {
                ...
                let seed = x.to_bits() ^ y.to_bits() ^ vx.to_bits() ^ vy.to_bits() ^ (i as u32 * 0x85EB_CA6B);
            ...
            out[ix] = x; out[ix + 1] = y; out[ix + 2] = vx; out[ix + 3] = vy;
        }
        out
    }
    ...
    pub fn delaunay_indices(points_flat: &[f32]) -> Vec<u32> {
        ...
        for i in (0..points_flat.len()).step_by(4) {
            if i + 1 < points_flat.len() {
                pts.push(Pt { x: points_flat[i], y: points_flat[i + 1] });
            }
        }
        ...
        for t in tris {
            out.push(t.a as u32); out.push(t.b as u32); out.push(t.c as u32);
        }
        out
    }
    ...
    pub fn voronoi_edges(points_flat: &[f32]) -> Vec<f32> {
        ...
        for (a, b) in segs { out.push(a.x); out.push(a.y); out.push(b.x); out.push(b.y); }
        out
    }
}
